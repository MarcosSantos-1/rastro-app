...
import { Bot, Expand, MapPin, Trash2 } from "lucide-react";
    ...
    <>
      ...
      <AnimatedModal
        ...
        title={
          <span className="inline-flex items-center gap-2">
            <MdiIcon
              path={CATEGORIA_MDI[local.categoria]}
              className="h-5 w-5 text-[var(--accent)]"
            />
            {categoriaLabel(local.categoria)}
          </span>
        }
      ...
      <div className="space-y-5">
        ...
        <div className={`rounded-xl border p-4 ${iaBoxClass}`}>
          <div className="mb-2 flex items-center gap-2 font-eyebrow text-[var(--muted)]">
        ...
        <div>
          <p className="mb-2 font-eyebrow text-[var(--muted)]">
            Fotos ({fotos.length})
          </p>
          {fotos.length === 0 ? (
              ...
              {fotos.map((url, i) => (
                <div key={url} className="relative">
                  ...
                  >
                    ...
                    <span className="pointer-events-none absolute inset-0 flex items-center justify-center bg-[rgba(4,16,10,0.28)] opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-[#0C1A13] shadow-sm">
                  ...
                  <button
                    ...
                    className="absolute right-2 top-2 z-10 inline-flex items-center gap-1 rounded-full bg-black/55 px-2 py-1 text-[11px] font-semibold text-white backdrop-blur hover:bg-red-600/90 disabled:opacity-50"
                  >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-full px-4 py-2 text-sm font-semibold text-[var(--muted)] hover:bg-[var(--accent-soft)] hover:text-[var(--foreground)]"
          >
            Fechar
          </button>
        </div>
...
function Meta({
  ...
  return (
    <div className={`rounded-xl border border-[var(--border)] bg-[var(--surface)]/60 px-3.5 py-3 ${className ?? ""}`}>
      <dt className="font-eyebrow flex items-center gap-1.5 text-[var(--muted)]">
        {icon}
        {label}
      </dt>
